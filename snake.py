...
def printFruit(screen):
    coords = (random.randint(1, screen.width-2),
              random.randint(1, screen.height-2))
    while(screen[coords[1]][coords[0]] != ' '):
        coords = (random.randint(1, screen.width),
                  random.randint(1, screen.height))
...
class Snake:
    def __init__(self, pantalla, charini='O', charfini='X'):
        ...
        self.pantalla.insertChar(
            self.coords_list[0][0], self.coords_list[0][1])
